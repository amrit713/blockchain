...
fn test_valid_transaction_creation_and_execution() {
    ...
    let tx = Transaction::new(&alice_keypair, bob_keypair.public_key(), 300, 0).unwrap();
...
fn test_tampered_transaction_fails_verification() {
    ...
    let mut tx = Transaction::new(&alice_keypair, bob_keypair.public_key(), 100, 0).unwrap();
...
fn test_zero_amount_and_self_transfer_rejected() {
    ...
    let zero_res = Transaction::new(&alice_keypair, bob_keypair.public_key(), 0, 0);
    assert_eq!(zero_res, Err(TransactionError::ZeroAmount));

    let self_res = Transaction::new(&alice_keypair, alice_keypair.public_key(), 100, 0);
...
fn test_insufficient_balance_rejection() {
    ...
    let tx = Transaction::new(&alice_keypair, bob_keypair.public_key(), 500, 0).unwrap();
...
fn test_invalid_nonce_rejection() {
    ...
    let tx = Transaction::new(&alice_keypair, bob_keypair.public_key(), 100, 0).unwrap();
